...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), settings(QCoreApplication::applicationDirPath()+"/settings.ini", QSettings::IniFormat)
{
...
void MainWindow::setCompleter(CodeEditor *editor)
{
    ...
    configreader =new ConfigReader(QCoreApplication::applicationDirPath()+"/syntax/"+fileinfo.suffix().toLower()+".json");
...
void MainWindow::applyCurrentSettings(CodeEditor *editor)
{
    ...
    QColor lineNumberAreaColor = settings.value("linenumberareaColor", QColor(Qt::white)).value<QColor>();
    QColor blockNumberColor = settings.value("blocknumberColor", QColor(Qt::white)).value<QColor>();
    QColor lineColor = settings.value("lineColor", QColor(Qt::white)).value<QColor>();
    QFont font = settings.value("font", QFont("Monospace", 12)).value<QFont>();

    editor->setFont(font);
    editor->palette.setColor(QPalette::Base,backgroundColor);
    editor->lineNumberAreaColor = lineNumberAreaColor;
    editor->blockNumberColor = blockNumberColor;
    editor->lineColor = lineColor;
...
void MainWindow::newFile()
{
    CodeEditor *editor = new CodeEditor(this,
                                        ...
                                        settings.value("linenumberareaColor", QColor(Qt::white)).value<QColor>(),
                                        settings.value("blocknumberColor", QColor(Qt::white)).value<QColor>(),
...
void MainWindow::showSettingsDialog()
{
    ...
    dialog.initColorDialogs(settings.value("backgroundColor", QColor(Qt::white)).value<QColor>(),
                            settings.value("linenumberareaColor", QColor(Qt::white)).value<QColor>(),
                            settings.value("blocknumberColor", QColor(Qt::white)).value<QColor>(),
                            settings.value("lineColor", QColor(Qt::white)).value<QColor>());
...
void MainWindow::applySettings(const QFont &font,
                               ...
                               const QColor &lineColor)
{
    ...
    settings.setValue("lineNumberAreaColor", lineNumberAreaColor);
    settings.setValue("blockNumberColor", blockNumberColor);
    ...
    for (int i = 0; i < tabWidget->count(); ++i) {
        ...
        if (editor) {
            MainWindow::applyCurrentSettings(editor);
        }
    }
}
...
{
    MainWindow::openFile(QCoreApplication::applicationDirPath()+"/apple_cat.txt");
}
